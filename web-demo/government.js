// MA-ISA Government Systems Demo - Multi-Dimensional Integrity for Critical Infrastructure
...
// Government-specific configuration with policies and constraints
const config = {
    dimensions: [
        { id: 0, name: "Secure Communications", color: "#60a5fa", threshold: 200, weight: 3.0, strategy: "Quarantine", safetyRelevant: true },
        { id: 1, name: "Inter-Agency Data Transfer", color: "#f472b6", threshold: 250, weight: 2.5, strategy: "ImmediateHeal", safetyRelevant: true },
        { id: 2, name: "Classified Document Handling", color: "#ef4444", threshold: 150, weight: 3.5, strategy: "Quarantine", safetyRelevant: true },
        { id: 3, name: "Critical Infrastructure Monitoring", color: "#fbbf24", threshold: 300, weight: 2.0, strategy: "ImmediateHeal", safetyRelevant: true }
    ],
    constraints: [
        {
            type: "MaxRatio",
            dimensions: [0, 2],
            ratio: 1.3,
            description: "Secure communications divergence must not exceed 1.3x classified document divergence"
        },
        {
            type: "SumBelow",
            dimensions: [0, 1, 2, 3],
            maxSum: 800,
            description: "Total divergence across all critical systems must stay below 800"
        },
        {
            type: "MaxRatio",
            dimensions: [1, 3],
            ratio: 1.5,
            description: "Inter-agency transfer divergence must not exceed 1.5x infrastructure monitoring divergence"
        }
    ],
    strategies: {
...
async function initializeWasm() {
    try {
        ...
        updateStatus();
        initializeMindMap();
        logEvent('SYSTEM', 'Government systems initialized - All dimensions operational', 0);
...
function initializeMindMap() {
    ...
    config.dimensions.forEach((dim, index) => {
        ...
        const dimNode = new Node(x, y, 'dimension', dim.name.split(' ')[0], {
            dimensionId: dim.id,
            color: dim.color,
            eventCount: 0,
            quarantined: false
...
window.recordEvent = function(dimensionId) {
    ...
    // Accumulate on both accumulators with correct signature
...
function generateEventData(dimensionId) {
    switch (dimensionId) {
        case 0: // Secure Communications
            const emailTypes = ['classified_email', 'encrypted_message', 'secure_video_call', 'diplomatic_cable'];
            const agencies = ['DoD', 'State_Dept', 'CIA', 'FBI', 'NSA', 'DHS'];
            const fromAgency = agencies[Math.floor(Math.random() * agencies.length)];
            const toAgency = agencies[Math.floor(Math.random() * agencies.length)];
            return `${emailTypes[Math.floor(Math.random() * emailTypes.length)]}_${fromAgency}_to_${toAgency}_${Math.floor(Math.random() * 10000)}`;
        case 1: // Inter-Agency Data Transfer
            const transferTypes = ['intel_report', 'case_file', 'threat_assessment', 'joint_operation_data', 'surveillance_logs'];
            const protocols = ['SIPRNET', 'JWICS', 'NSANet', 'GWAN'];
            return `transfer_${transferTypes[Math.floor(Math.random() * transferTypes.length)]}_via_${protocols[Math.floor(Math.random() * protocols.length)]}_${Math.floor(Math.random() * 100000)}`;
        case 2: // Classified Document Handling
            const docTypes = ['TOP_SECRET', 'SECRET', 'CONFIDENTIAL', 'NOFORN', 'EYES_ONLY'];
            const actions = ['access', 'modify', 'print', 'transmit', 'archive', 'destroy'];
            const classification = docTypes[Math.floor(Math.random() * docTypes.length)];
            return `doc_${actions[Math.floor(Math.random() * actions.length)]}_${classification}_SCI_${Math.floor(Math.random() * 100000)}`;
        case 3: // Critical Infrastructure Monitoring
            const infraTypes = ['power_grid', 'water_supply', 'telecom_network', 'transportation', 'financial_system', 'emergency_services'];
            const events = ['status_check', 'anomaly_detected', 'security_scan', 'backup_verification', 'failover_test'];
            return `infra_${events[Math.floor(Math.random() * events.length)]}_${infraTypes[Math.floor(Math.random() * infraTypes.length)]}_${Math.floor(Math.random() * 1000)}`;
        default:
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[INIT] Starting government systems demo...');
    initializeWasm();
});
